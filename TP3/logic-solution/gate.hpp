...
namespace gate {
  ...
  bool value(ref_wire w) {return std::get<0>(*w);}
  void link(ref_wire w, ref_action a) {std::get<1>(*w).push_back(a);}

  void set(ref_wire w, bool v) {
    if(v == value(w)) return;
    std::get<0>(*w) = v;
  ...
  double now(ref_agenda a) {return std::get<0>(*a);}
  void notify(ref_agenda a, ref_wire w, bool status, double delay) {std::get<1>(*a).insert({now(a) + delay, {w, status}});}
  
  bool next(ref_agenda a) {
  ...
  void flush(ref_agenda a) {while(next(a));}
  void flush(ref_agenda a, unsigned int nb_steps) {unsigned int n = 0; while((n++ < nb_steps) && next(a));}


  auto PROBE(ref_agenda a, const std::string& msg, ref_wire w) {
