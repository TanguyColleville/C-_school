...
void raz_ref(num& x) {
  fun_scope;
  std::cout<<"Here is the value of x : "<<x<<std::endl;
  x = 3; // Since the argument type is num&, x ***is*** j. 
  // on dit que x et la variable d'entrée pointe vers la même case mémoire 
  //donc si je dis que x=0 bah la variable d'entrée sera aussi égale à 0
}
