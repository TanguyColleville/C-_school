...
bool petri::Network::operator()() {
  std::vector<Transition*> activables;// ça va être une liste de pointeur des transitions activables 
  auto out = std::back_inserter(activables);// une structure bizarre
  for(auto& t : transitions)// je prends mes transitions
    if(t) *(out++) = &t;// si elle est activable j'ajouteu une référence dessus car 
    // on voudra utiliser les transitions du réseau pas des copies.

  if(activables.size() == 0)// si j'en ai aucune activable
    return false;// je renvoie faut je ne peux pas faire mon activation 
  // si je peux faire mon activation 
  auto id = std::uniform_int_distribution<std::size_t>(0, activables.size()-1)(gen);
  Transition& t = *(activables[id]);// j'active une transition parmi les bonnes 
  // de manière aléatoire attention on prend la valeur du pointeur pour que ce soit l'activation
  // de notre réseau et c'est une référence car on veut activer la transition du réseau pas 
  // une copie.
